...
class initial_frame():
    def __init__(self, master):
        self.master = master
        self.import_button = tk.Button(self.master, text = 'Import', command = self.import_)
        ...
        self.threshold_entry = tk.Entry(self.master, text = 'Guassian sigma', textvariable=self.threshold)
        self.threshold_entry.grid(row=1, column=3)
        self.calibrate_button = tk.Button(self.master, text = 'Calibrate', command = self.calibrate_)
        ...
        self.shift_entry = tk.Entry(self.master, text = 'Shift', textvariable=self.shift)
        ...
        self.depth_entry = tk.Entry(self.master, text = 'Depth', textvariable=self.depth)
        self.depth_entry.grid(row=2, column=3)
        self.mask_button = tk.Button(self.master, text = 'Mask', command = self.mask_)
        self.mask_button.grid(row=2, column=4)
        self.export_button = tk.Button(self.master, text = 'Export', command = self.export_)
    ...
    def calibrate_(self):
        ...
        self.calibration_app = calibrate_window(self.calibrate_window, self.data, self.binary)

    def threshold_(self, *args):
        filtered = np.copy(self.data)
        for idx, plane in enumerate(filtered):
            filtered[idx] = filters.gaussian_filter(plane, int(self.gauss_sigma.get()))
    ...
    def mask_(self):
        ...
        padding = np.zeros((shift_mag, self.binary.shape[1], self.binary.shape[2]))
...
class calibrate_window():
    def __init__(self, master, data, binary):
        ...
        self.plane = 0
        self.show_img()
        self.plane_chooser = tk.Scale(self.master, from_=0, to=len(self.binary)-1, command=self.plane_change)
    ...
    def show_img(self, *args):
        ...
        self.img = Image.fromarray(self.data[self.plane]).resize((500, 500), Image.ANTIALIAS)
        ...
        self.bin_img = Image.fromarray(self.binary[self.plane]).resize((500, 500), Image.ANTIALIAS)
        ...
        self.calibrate_canvas.create_image(0, 0, image=self.bin_img , anchor="nw")
